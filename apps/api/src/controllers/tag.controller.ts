...
export class TagController {
  ...
  async list(siteId: string, pagination: PaginationParams & { search?: string }): Promise<PaginatedResult<Tag>> {
  ...
  async create(siteId: string, input: { name: string; slug?: string; description?: string; featureImage?: string }): Promise<Tag> {
    return this.tagService.createTag(siteId, input)
  }

  async update(siteId: string, id: string, input: Partial<{ name: string; slug: string; description: string | null; featureImage: string | null }>): Promise<Tag> {
