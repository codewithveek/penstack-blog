...
import type { PostWithAuthorsAndTags, PostFindManyParams, PaginatedResult, PostAuthorInput } from "@cms/core/types/repositories"
import type { z } from "zod"
import type { createPostSchema, updatePostSchema, listPostsQuerySchema } from "@cms/core/validators/post"
...
export class PostController {
  ...
  async getById(siteId: string, postId: string): Promise<PostWithAuthorsAndTags> {
    return this.postService.getById(siteId, postId)
  }

  async getBySlug(siteId: string, slug: string): Promise<PostWithAuthorsAndTags> {
    return this.postService.getBySlug(siteId, slug)
  }

  async getByPermalink(siteId: string, permalink: string): Promise<PostWithAuthorsAndTags> {
    return this.postService.getByPermalink(siteId, permalink)
  }

  async list(siteId: string, query: ListPostsQuery): Promise<PaginatedResult<PostWithAuthorsAndTags>> {
    return this.postService.listPosts(siteId, query as PostFindManyParams)
  }

  async publish(siteId: string, postId: string): Promise<PostWithAuthorsAndTags> {
    return this.postService.publishPost(siteId, postId)
  }

  async schedule(siteId: string, postId: string, scheduledAt: Date): Promise<void> {
    return this.postService.schedulePost(siteId, postId, scheduledAt)
  }

  async unpublish(siteId: string, postId: string): Promise<PostWithAuthorsAndTags> {
  ...
  async setAuthors(siteId: string, postId: string, authors: PostAuthorInput[]): Promise<void> {
    return this.postService.updatePostAuthors(siteId, postId, authors)
  }

  async setTags(siteId: string, postId: string, tagIds: string[]): Promise<void> {
