...
export class WebhookController {
  ...
  async create(siteId: string, input: { targetUrl: string; events: string[]; secret?: string }): Promise<Webhook> {
    return this.webhookService.createWebhook(siteId, input)
  }

  async update(siteId: string, id: string, input: Partial<{ targetUrl: string; events: string[]; active: boolean }>): Promise<Webhook> {
  ...
  async listDeliveries(siteId: string, webhookId: string, pagination: PaginationParams): Promise<PaginatedResult<WebhookDelivery>> {
