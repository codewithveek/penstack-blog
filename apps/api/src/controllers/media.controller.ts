...
export class MediaController {
  ...
  async upload(siteId: string, uploadedById: string, fileBuffer: Buffer, filename: string, mimeType: string): Promise<MediaAsset> {
    return this.mediaService.upload(siteId, uploadedById, fileBuffer, filename, mimeType)
  }
  ...
  async list(siteId: string, pagination: PaginationParams & { type?: MediaAsset["type"] }): Promise<PaginatedResult<MediaAsset>> {
  ...
  async getStorageUsage(siteId: string): Promise<{ bytes: number; mb: number }> {
