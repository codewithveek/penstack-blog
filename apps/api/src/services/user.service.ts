...
export class UserService {
  ...
  async listUsers(siteId: string, pagination: PaginationParams): Promise<PaginatedResult<User>> {
  ...
  private async ensureUniqueSlug(siteId: string, slug: string): Promise<string> {
    let candidate = slug
    let attempt = 0
    while (true) {
      const existing = await this.userRepo.findByEmail(siteId, candidate)
        .catch(() => null)
