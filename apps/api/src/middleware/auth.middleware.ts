...
export function requireAdminAuth(): MiddlewareHandler {
  return async (c, next) => {
    ...
    if (session?.user && session?.session) {
      ...
      c.set(
        ...
        ((session.user as { role?: string })
          .role as (typeof c)["var"]["role"]) ?? "author"
      );
      await next();
      return;
    }
    ...
    if (authHeader.startsWith("Bearer ")) {
      ...
      const settingsService: SettingsService | undefined = (
        c as unknown as { settingsService?: SettingsService }
      ).settingsService;
...
export function requireRole(
  ...roles: Array<"owner" | "admin" | "editor" | "author" | "contributor">
): MiddlewareHandler {
  return async (c, next) => {
